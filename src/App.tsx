...
function App() {
  ...
  useEffect(() => {
    ...
    const generate = async () => {
      ...
      if (needsNewOriginal && (pathChanged || !transformedOriginalUrl)) {
        // Clear old URL if path changed
        if (pathChanged && transformedOriginalUrl) {
          setTransformedOriginalUrl(null);
        }
        ...
        try {
          ...
          if (isEffectActive && selectedImage?.path) {
            const blob = new Blob([imageData], { type: 'image/jpeg' });
            objectUrl = URL.createObjectURL(blob);
            transformedOriginalPathRef.current = selectedImage.path;
  ...
  // Update ISO multiplier for denoiser when image changes or auto-ISO mode changes
  useEffect(() => {
    if (!adjustments.denoiseAutoIso) {
      // If auto ISO is disabled, use 1.0 multiplier
      if (adjustments.denoiseIsoMultiplier !== 1.0) {
        setAdjustments((prev: Adjustments) => ({ ...prev, denoiseIsoMultiplier: 1.0 }));
      }
      return;
    }

    // Calculate ISO multiplier from EXIF
    const isoStr = selectedImage?.exif?.ISOSpeedRatings;
    ...
    if (iso && iso > 0) {
      ...
      if (iso <= 400) {
        multiplier = 0.3 + (logIso / 2) * 0.2;
      } else if (iso <= 1600) {
        multiplier = 0.5 + ((logIso - 2) / 2) * 0.3;
      } else if (iso <= 6400) {
        multiplier = 0.8 + ((logIso - 4) / 2) * 0.4;
      } else {
        multiplier = Math.min(1.5, 1.2 + ((logIso - 6) / 2) * 0.3);
      }
    }

    if (adjustments.denoiseIsoMultiplier !== multiplier) {
      setAdjustments((prev: Adjustments) => ({ ...prev, denoiseIsoMultiplier: multiplier }));
    }
  }, [selectedImage?.exif?.ISOSpeedRatings, adjustments.denoiseAutoIso]);
  ...
  const applyAdjustments = useCallback(
    ...
    }, 150),  // Increased from 50ms to reduce processing during rapid adjustments
  ...
  const handleApplyUpscale = async () => {
    ...
    try {
      ...
      setError(`Failed to upscale image: ${err}`);
    } finally {
                    ...
                    >
                      {renderedRightPanel === Panel.Adjustments && (
                        <Controls
                          ...
                          isUpscaled={selectedImage?.path?.includes('_upscaled') ?? false}
                          isUpscaling={isUpscaling}
                          imageIso={selectedImage?.exif?.ISOSpeedRatings ? parseInt(selectedImage.exif.ISOSpeedRatings, 10) : null}
                        />
                      )}
