...
use crate::raw_processing::{develop_raw_image, develop_raw_image_with_exif};
...
/// Load an image and return EXIF data if available (especially for RAW files)
/// Returns (image, Some(exif_map)) for RAW files, (image, None) for other formats
pub fn load_base_image_with_exif(
    bytes: &[u8],
    path_for_ext_check: &str,
    use_fast_raw_dev: bool,
    highlight_compression: f32,
) -> Result<(DynamicImage, Option<HashMap<String, String>>)> {
    let path = std::path::Path::new(path_for_ext_check);

    // EXR files
    if path.extension().and_then(|s| s.to_str()).map_or(false, |s| s.eq_ignore_ascii_case("exr")) {
        return Ok((load_exr_from_bytes(bytes)?, None));
    }

    // QOI files
    if path.extension().and_then(|s| s.to_str()).map_or(false, |s| s.eq_ignore_ascii_case("qoi")) {
        return Ok((load_qoi_from_bytes(bytes)?, None));
    }

    // RAW files - use rawler's EXIF extraction
    if is_raw_file(path_for_ext_check) {
        match panic::catch_unwind(|| develop_raw_image_with_exif(bytes, use_fast_raw_dev, highlight_compression)) {
            Ok(Ok((image, exif))) => Ok((image, Some(exif))),
            Ok(Err(e)) => {
                log::warn!("Error developing RAW file '{}': {}", path_for_ext_check, e);
                Err(e)
            }
        ...
        // Standard image formats - no EXIF from this path (use kamadak-exif in main.rs)
        Ok((load_image_with_orientation(bytes)?, None))
    }
}
